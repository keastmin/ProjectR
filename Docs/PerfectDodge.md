...
기본값 0.2 / 0.08초 / 0.15초는 Inspector 초기값이며 로직 내부의 고정 상수가 아닙니다.
실행 중에도 다음 완벽 회피부터 새 설정을 사용합니다. Unity Play Mode에서 변경한 값은 종료하면 되돌아가므로 영구 설정은 Edit Mode에서 저장합니다.

## 동작

- 회피 성공 시 대상 적을 저장하고 플레이어, 활성 적, 등록된 전투 VFX에 슬로우를 적용합니다.
- 회피 클립의 `OnPerfectDodgeEndInvoke`가 반격 구간을 닫고 FadeOut을 시작합니다. 슬로우가 강할수록 이 이벤트까지의 실제 시간도 길어집니다.
- 반격 구간에서 좌클릭하면 FadeIn/FadeOut을 기다리지 않고 자신의 슬로우 요청을 제거한 뒤 `DodgeAttackStartState`에 진입합니다.
- 완벽 회피 성공부터 `OnPerfectDodgeEndInvoke`까지 무적이며, 반격하면 DodgeAttack의 Start/Loop/End 전체로 무적이 이어집니다. 일반 회피, 자연 종료 후 FadeOut만 남은 구간, 반격을 벗어난 상태는 무적이 아닙니다.
